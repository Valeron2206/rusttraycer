...
#[test]
fn migrations_0001_to_0010_byte_identical_to_ce84e59() {
    ...
    for name in names {
        ...
        let out = Command::new("git")
            .args([
                "show",
                &format!("ce84e59:crates/rt-storage/migrations/{name}"),
            ])
            .current_dir(&root)
            .output()
            .expect("git show");
        assert!(
            out.status.success(),
            "git show ce84e59:{name} failed: {}",
            String::from_utf8_lossy(&out.stderr)
        );
        assert_eq!(
            current, out.stdout,
            "{name} must be byte-identical to ce84e59"
        );
    }
}
