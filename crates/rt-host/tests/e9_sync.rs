...
#[test]
fn migrations_0001_to_0008_byte_identical_vs_e165227() {
    let root = repo_root();
    let names = [
        "0001_init.sql",
        "0002_worktrees.sql",
    ...
    for name in names {
        ...
        let out = std::process::Command::new("git")
            .args([
                "show",
                &format!("e165227:crates/rt-storage/migrations/{name}"),
            ])
            .current_dir(&root)
            .output()
            .unwrap();
        assert!(
            out.status.success(),
            "git show e165227:{} failed: {}",
            name,
            String::from_utf8_lossy(&out.stderr)
        );
        assert_eq!(
            current, out.stdout,
            "{name} must be byte-identical to e165227"
        );
    }
}
