...
#[test]
fn schema_0009_is_nine_no_secret_columns_0001_to_0008_byte_identical() {
    ...
    for name in names {
        ...
        let out = Command::new("git")
            .args([
                "show",
                &format!("c07ce66:crates/rt-storage/migrations/{name}"),
            ])
            .current_dir(&root)
            .output()
            .expect("git show");
        assert!(
            out.status.success(),
            "git show c07ce66:{name} failed: {}",
            String::from_utf8_lossy(&out.stderr)
        );
        assert_eq!(
            current, out.stdout,
            "{name} must be byte-identical to c07ce66"
        );
    }
