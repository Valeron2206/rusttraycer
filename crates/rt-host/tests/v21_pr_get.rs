...
#[test]
fn migrations_0001_to_0010_byte_identical_to_24a4dec() {
    ...
    for name in names {
        ...
        let out = Command::new("git")
            .args([
                "show",
                &format!("24a4dec:crates/rt-storage/migrations/{name}"),
            ])
            .current_dir(&root)
            .output()
            .expect("git show");
        assert!(
            out.status.success(),
            "git show 24a4dec:{name} failed: {}",
            String::from_utf8_lossy(&out.stderr)
        );
        assert_eq!(
            current, out.stdout,
            "{name} must be byte-identical to 24a4dec"
        );
    }
}
